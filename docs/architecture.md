...
Status is `accepted` unless stated. An `accepted` ADR is binding on every plan,
change and fix. A `proposed` ADR is not yet binding and names the sprint that
must accept or amend it. An ADR changes only through a later ADR that names the
one it amends or supersedes.
...
| Relates to | `NFR-REPO-003`, `REQ-E2E-001`, `REQ-RUNTIME-001`, `REQ-RUNTIME-003` |

**Context.** Earlier scaffold revisions planned a typed command registry: a
...
**Decision.** Operations are shared async service functions written in project
...
**Consequences.** Which routes exist, which store a route uses and what data
...
that check belongs to sc-lint (ADR-009).

**Rejected.** A command registry with generated adapters; a macro system; a
portable abstraction over the three surfaces.

**Enforced by.** `arch-qa` review; `NFR-REPO-003`; `sc-runtime` boundary
...
| Relates to | `REQ-REPO-001`, `REQ-REPO-002`, `REQ-REPO-003`, `REQ-RELEASE-002` |

**Context.** Rev 4 planned to start the reusable crates as path dependencies
...
**Decision.** The four crates are built as separate workspace crates from the
...
**Consequences.** Generated projects upgrade with `cargo update`. The template
...
**Rejected.** Path dependencies copied into projects; one monolithic
`sc-runtime` crate; a repository per crate from day one (loses same-commit
template CI).

**Enforced by.** `req-qa` on `REQ-REPO-003`; the fixture matrix.

### ADR-003 The daemon is always running and everything speaks HTTP
...
| Relates to | `REQ-E2E-002`, `REQ-RUNTIME-002`, `REQ-TRANSPORT-006` |

**Context.** A CLI that can open the database itself when no daemon is running
...
**Decision.** The daemon is the only process that opens the local database and
...
**Consequences.** One code path to SQL and one writer. Every CLI command needs
...
**Rejected.** A direct-database CLI fallback; PID files or port probing as the
singleton mechanism.

**Enforced by.** `NFR-REPO-001` (the CLI cannot link sqlx); the
`DAEMON.NOT_RUNNING` test in `REQ-E2E-002`.
...
| Relates to | `NFR-REPO-001`, `REQ-REPO-002`, `REQ-REPO-005`, `NFR-TRANSPORT-001`, `NFR-COMMAND-001`, `NFR-RUNTIME-003` |

**Context.** Two properties pull against each other. Each of `sc-config`,
...
**Decision.** `sc-config`, `sc-transport` and `sc-command` are standalone and
...
**Consequences.** A CLI depends on the default builds and links none of the
...
project's CLI maps it into an `OpError` (ADR-TRANSPORT-003). Feature-gated code
must be tested with and without the feature.

**Rejected.** Splitting each crate into `-client` and `-server` crates (twice
...
`cargo tree` evidence for `NFR-REPO-001`.
...
| Relates to | `REQ-TEMPLATE-009`, `NFR-CONFIG-002`, `NFR-COMMAND-002`, `NFR-RUNTIME-002` |

**Context.** `sc-observability` 1.2.x is the SC logging standard and every
...
**Decision.** `sc-observability` and its OTel export crate stay independent of
...
**Consequences.** `sc-config` has no observability dependency and can be used
...
**Rejected.** `sc-runtime` initialising observability; a logging facade inside
these crates.

**Enforced by.** `forbidden_edges` to `sc-observability` in each crate's
boundary manifest; `arch-qa`.

### ADR-006 One router carries REST, OpenAPI and MCP

| Field | Value |
|---|---|
| Status | proposed; accepted or amended by sprint aa-1 |
| Date | 2026-09-19 |
| Source | design decisions 3, 15, 18; Verify items 19, 20 |
| Relates to | `REQ-SPIKE-001`, `REQ-SPIKE-002`, `REQ-E2E-005`, `REQ-RUNTIME-004`, `NFR-REPO-007` |

**Context.** REST with an OpenAPI document and an MCP server are usually
...
**Decision.** REST routes and `openapi.json` come from `utoipa-axum`
...
**Consequences.** Stateless mode means the template carries no session store.
...
**Rejected.** `mcpkit-axum`; `aide`; a separate MCP process or port.
...
| Relates to | `REQ-TEMPLATE-008`, `REQ-E2E-001`, `REQ-RUNTIME-003` |

**Context.** SC projects commonly pair SQLite (fast local traffic) with
...
**Decision.** A store is one crate bound to one sqlx driver, with its own pool,
...
**Consequences.** Both backends build in one workspace. Each service function
...
**Rejected.** A portable query layer; one store crate with backend features;
the write actor as the default.

**Enforced by.** The generated workspace's dependency edges
(`REQ-TEMPLATE-001`); `arch-qa`.
...
| Relates to | `REQ-ANSWERS-001..003`, `REQ-DRIVER-001..003`, `REQ-TEMPLATE-003..006`, `REQ-WIZARD-001..003`, `REQ-RELEASE-001` |

**Context.** `p3-nuget-template` already proved a pipeline: a Wyvern wizard
...
**Decision.** `wizard/answers.schema.json` is the single, versioned options
...
**Consequences.** Use the appropriate tool for each job, and write no render
...
**Rejected.** A custom render engine; `cargo-generate-action` matrix; Liquid
...
| Relates to | `REQ-TEMPLATE-007`, `NFR-REPO-006`, `REQ-REPO-004`, `REQ-REPO-005`, `REQ-ANSWERS-001` |

**Context.** Rev 4 wanted adapter-drift enforcement: tooling that keeps REST,
...
**Decision.** The template carries no lint configuration, boundary rules,
...
**Consequences.** When sc-lint's rules change, generated projects pick that up
...
**Rejected.** Surface snapshots of `openapi.json`, MCP `tools/list` and the
clap model in this repo; lint configuration in the template.

**Enforced by.** `NFR-REPO-006`; `req-qa`.
...
| Relates to | `REQ-E2E-001`, `REQ-TEMPLATE-001`, `REQ-TEMPLATE-002` |

**Context.** The draft planned to generate the CLI's client with `progenitor`
...
**Decision.** The template example derives `Serialize`, `Deserialize`,
...
**Consequences.** Daemon and CLI compile against the same structs, which gives
...
**Rejected.** `progenitor`; per-surface request structs in the example.
...
| Relates to | `NFR-REPO-009`, `NFR-CONFIG-001`, `NFR-TRANSPORT-004`, `NFR-COMMAND-003`, `NFR-RUNTIME-004`, `REQ-E2E-003` |

**Context.** The design states for `sc-config` that every method returns a
...
**Decision.** Public library APIs return `Result<T, E>` with one typed error
...
**Consequences.** Callers can branch on error variants; the generated `main.rs`
turns them into exit codes. Each crate carries a small error enum and the tests
for its variants.

**Rejected.** `anyhow`-style opaque errors in public APIs; panicking
constructors.
